 ...
 *
 * A DELETE that SUCCEEDED on disk but whose `live.failed` is non-empty takes the
 * same path with a one-line notice ("Cleared on disk; N live session(s) still
 * hold cookies"): the set is gone, so the chip leaves, but a live session may
 * stay signed in until it closes, and silence there would misreport "clear".
 * The row stays mounted (disabled — there is nothing further to clear) while
 * that notice is showing, so the outcome does not vanish with the chip.
 *
 * Hidden for a non-owner and when nothing is imported (and no notice is up).
 */
export function CookieClearMenuItem({ cookies }: { cookies: Cookies }) {
  ...
  const onSelect = useCallback((event: Event) => {
    ...
    setArmed(false)
    cookies.clear().then((result) => {
      // The on-disk set is gone; say so about the live sessions that kept it.
      const stillHolding = Object.keys(result.live?.failed ?? {}).length
      if (stillHolding > 0) {
        setFailure(i18nT('components.webPreviewPanel.cookies_clear_live_failed', {
          count: fmtNumber(stillHolding),
        }))
      }
    }, (e: unknown) => {
  ...
  const present = Boolean(cookies.data?.present)
  // A fresh import supersedes a lingering "still hold cookies" notice.
  useEffect(() => { if (present) setFailure(null) }, [present])
  if (cookies.forbidden || (!present && !failure)) return null
  ...
  return (
    <>
      <DropdownMenuItem
        onSelect={onSelect}
        disabled={cookies.clearing || !present}
